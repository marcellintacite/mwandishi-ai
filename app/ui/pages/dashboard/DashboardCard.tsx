...
export default async function DashboardCard({ data }: Props) {
  const session = await getServerSession();
  const number = await prisma.prompt.count({
    where: {
      user: {
        email: session?.user?.email as string,
      },
      type: data.title === "Composition" ? "compose" : "cv",
    },
    ...
    >
      <div className="flex items-center h-full">
        ...
        <div className="h-full flex justify-center items-center w-full ">
          <p className="text-3xl font-extrabold text-gray-700">{number}</p>
        </div>
